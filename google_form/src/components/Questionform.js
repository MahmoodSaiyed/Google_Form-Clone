...
export default function Questionform({onDataFetch}) {
  ...
  useEffect(() => {
    ...
    function addQuestionType(i, type) {
      ...
      if (type === 'text') {
        qs[i].options = []; // Clear options array for text type
      } else if (qs[i].options.length === 0) {
        qs[i].options.push({ optionText: 'Option1' }); // Add a default option for other types
      }
      setQuestion(qs);
    }
    //  Change Option Value
                ...
                <AccordionDetails className='add_question'>
                    ...
                    {ques.options.length <5 ?(
                        <div className='add_question_body'>
                            ...
                            label={ ques.questionType!=='text'?(   <div style={{display: 'flex', alignItems: 'center'}}>
